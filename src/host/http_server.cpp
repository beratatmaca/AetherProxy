#include "host/http_server.hpp"
#include "common/embedded_assets.hpp"
#include <sys/socket.h>
#include <sys/time.h>
#include <netinet/in.h>
#include <unistd.h>
#include <sstream>
#include <iostream>
#include <cstring>
#include <cstdlib>
#include <algorithm>

static std::string queryParam(const std::string &request, const std::string &key) {
    size_t lineEnd = request.find("\r\n");
    std::string line = request.substr(0, lineEnd);
    std::string marker = key + "=";
    size_t pos = line.find(marker);
    if (pos == std::string::npos) {
        return "";
    }
    size_t start = pos + marker.size();
    size_t end = std::min(line.find(' ', start), line.find('&', start));
    return line.substr(start, end - start);
}

static std::string urlDecode(const std::string &text) {
    std::string out;
    for (size_t i = 0; i < text.size(); ++i) {
        if (text[i] == '+') {
            out.push_back(' ');
        } else if (text[i] == '%' && i + 2 < text.size()) {
            auto hex = text.substr(i + 1, 2);
            char *end = nullptr;
            long v = std::strtol(hex.c_str(), &end, 16);
            if (end != nullptr && *end == '\0') {
                out.push_back(static_cast<char>(v));
                i += 2;
            } else {
                out.push_back(text[i]);
            }
        } else {
            out.push_back(text[i]);
        }
    }
    return out;
}

static void writeAll(int fd, const char *data, size_t len) {
    size_t sent = 0;
    while (sent < len) {
        ssize_t n = write(fd, data + sent, len - sent);
        if (n <= 0) {
            return;
        }
        sent += static_cast<size_t>(n);
    }
}

static std::string assetResponse(const std::vector<unsigned char> &asset, const char *contentType) {
    std::string content(asset.begin(), asset.end());
    std::ostringstream oss;
    oss << "HTTP/1.1 200 OK\r\nContent-Type: " << contentType << "\r\nContent-Length: " << content.size() << "\r\nConnection: close\r\n\r\n"
        << content;
    return oss.str();
}

HttpServer::HttpServer() = default;

HttpServer::~HttpServer() {
    stop();
}

void HttpServer::stop() {
    running = false;
    if (serverFd != -1) {
        shutdown(serverFd, SHUT_RDWR);
        close(serverFd);
        serverFd = -1;
    }
}

bool HttpServer::bindTo(uint16_t requestedPort, bool loopbackOnly) {
    serverFd = socket(AF_INET, SOCK_STREAM, 0);
    if (serverFd == -1) {
        return false;
    }

    int opt = 1;
    setsockopt(serverFd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));

    sockaddr_in addr{};
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = loopbackOnly ? htonl(INADDR_LOOPBACK) : INADDR_ANY;
    addr.sin_port = htons(requestedPort);

    if (bind(serverFd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        std::cerr << "Port " << requestedPort << " is busy. Server not started.\n" << std::flush;
        close(serverFd);
        serverFd = -1;
        return false;
    }

    if (listen(serverFd, 10) < 0) {
        close(serverFd);
        serverFd = -1;
        return false;
    }

    sockaddr_in bound{};
    socklen_t len = sizeof(bound);
    if (getsockname(serverFd, (struct sockaddr *)&bound, &len) == 0) {
        port = ntohs(bound.sin_port);
    } else {
        port = requestedPort;
    }
    return true;
}

uint16_t HttpServer::boundPort() const {
    return port;
}

void HttpServer::run(const std::function<std::string(std::string, std::string)> &getOffer,
                     const std::function<void(std::string, std::string)> &onAnswerReceived,
                     const std::function<void(const std::string &)> &onControl) {
    if (serverFd == -1) {
        return;
    }

    static const char *NO_CONTENT = "HTTP/1.1 204 No Content\r\nConnection: close\r\n\r\n";
    static const char *NOT_FOUND = "HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";

    running = true;
    while (running) {
        int clientFd = accept(serverFd, nullptr, nullptr);
        if (clientFd < 0) {
            if (!running)
                break;
            continue;
        }

        struct timeval rcvTimeout {
            .tv_sec = 5, .tv_usec = 0
        };
        setsockopt(clientFd, SOL_SOCKET, SO_RCVTIMEO, &rcvTimeout, sizeof(rcvTimeout));

        char buffer[4096] = {0};
        ssize_t bytesRead = read(clientFd, buffer, sizeof(buffer) - 1);
        if (bytesRead <= 0) {
            close(clientFd);
            continue;
        }

        std::string request(buffer);
        std::string response;

        if (request.find("GET / ") != std::string::npos || request.find("GET /?") != std::string::npos) {
            response = assetResponse(INDEX_HTML, "text/html");
        } else if (request.find("GET /ping") != std::string::npos) {
            if (onControl) {
                onControl("ping");
                if (request.find("ended=1") != std::string::npos) {
                    onControl("ended");
                }
            }
            response = NO_CONTENT;
        } else if (request.find("POST /bye") != std::string::npos) {
            if (onControl) {
                onControl("bye");
            }
            response = NO_CONTENT;
        } else if (request.find("GET /offer") != std::string::npos) {
            if (!getOffer) {
                response = NOT_FOUND;
            } else {
                std::string clientId = queryParam(request, "id");
                if (clientId.empty()) {
                    clientId = "client-unknown";
                }
                std::string name = urlDecode(queryParam(request, "name"));
                std::string offer = getOffer(clientId, name);
                std::ostringstream oss;
                if (offer.empty()) {
                    static const std::string full = "session full";
                    oss << "HTTP/1.1 503 Service Unavailable\r\nContent-Type: text/plain\r\nContent-Length: " << full.size()
                        << "\r\nConnection: close\r\n\r\n"
                        << full;
                } else {
                    oss << "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: " << offer.size()
                        << "\r\nConnection: close\r\n\r\n"
                        << offer;
                }
                response = oss.str();
            }
        } else if (request.find("GET /xterm.js") != std::string::npos) {
            response = assetResponse(XTERM_JS, "application/javascript");
        } else if (request.find("GET /xterm.css") != std::string::npos) {
            response = assetResponse(XTERM_CSS, "text/css");
        } else if (request.find("GET /xterm-addon-fit.js") != std::string::npos) {
            response = assetResponse(XTERM_ADDON_FIT_JS, "application/javascript");
        } else if (request.find("POST /answer") != std::string::npos) {
            if (!onAnswerReceived) {
                response = NOT_FOUND;
            } else {
                std::string clientId = queryParam(request, "id");
                if (clientId.empty()) {
                    clientId = "client-unknown";
                }
                size_t clPos = request.find("Content-Length:");
                size_t bodyPos = request.find("\r\n\r\n");
                size_t contentLength = 0;
                if (clPos != std::string::npos) {
                    size_t endLine = request.find("\r\n", clPos);
                    std::string clVal = request.substr(clPos + 15, endLine - (clPos + 15));
                    size_t first = clVal.find_first_not_of(" \t");
                    try {
                        contentLength = (first != std::string::npos) ? std::stoul(clVal.substr(first)) : 0;
                    } catch (const std::exception &) {
                        const char *bad = "HTTP/1.1 400 Bad Request\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
                        writeAll(clientFd, bad, std::strlen(bad));
                        close(clientFd);
                        continue;
                    }
                }
                std::string body;
                if (bodyPos != std::string::npos) {
                    body = request.substr(bodyPos + 4);
                }
                while (body.size() < contentLength) {
                    char readBuf[1024] = {0};
                    ssize_t n = read(clientFd, readBuf, sizeof(readBuf) - 1);
                    if (n <= 0)
                        break;
                    body.append(readBuf, n);
                }
                std::string sdpAnswer = body;
                response = "HTTP/1.1 200 OK\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
                writeAll(clientFd, response.data(), response.size());
                close(clientFd);
                onAnswerReceived(clientId, sdpAnswer);
                continue;
            }
        } else {
            response = NOT_FOUND;
        }

        if (!response.empty()) {
            writeAll(clientFd, response.data(), response.size());
        }
        close(clientFd);
    }
    stop();
}
